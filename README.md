...
## 🛠️ Deployment Instructions (For USB / Local Handover)

Follow these exact steps to present and deploy the project from any machine.

### Prerequisites
1. **AWS CLI:** Installed and configured (`aws configure`) with appropriate IAM credentials.
2. **Terraform:** v1.5.0+ installed.
3. **Node.js & npm:** Installed for the frontend.
4. **Python 3.10+:** Installed.

### Step 1: Provision Infrastructure (Terraform)
Navigate to the `infra` directory and apply the Terraform configuration. This creates the Database, S3 buckets, API Gateway, and Lambda.
```bash
cd infra
terraform init
terraform apply -auto-approve
```
*Note down the generated `api_url` and `file_storage_bucket` from the Terraform outputs.*

### Step 2: Deploy Backend (AWS Lambda)
Package the Python code and upload it to the newly created Lambda function:
```bash
cd ../backend
# Install dependencies for Amazon Linux environment
...
npm start
```
*(For production, run `npm run build` and sync the build folder to the S3 frontend bucket).*
...
- [x] **Frontend:** React application with a premium theme.
- [x] **Authentication:** Implemented secure login gateway.
- [x] **Specific Requirement:** Files are stored in S3 using **Pre-Signed URLs** generated by the backend.
